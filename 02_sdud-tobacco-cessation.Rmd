...
```{r, apiPull}
url <- 'https://data.medicaid.gov/api/1/datastore/query/6c114b2c-cb83-559b-832f-4d8b06d6c1b9/0'
...
medicaidEnrollmentKYQtr <- medicaidEnrollmentKYQtr[ , `:=` (quarter = fcase(enrollment_month %in% c('1', '2', '3'), 1,
                                                                               enrollment_month %in% c('4', '5', '6'), 2,
                                                                               enrollment_month %in% c('7', '8', '9'), 3,
                                                                               enrollment_month %in% c('10', '11', '12'), 4,
                                                                               default = 5))][ ,
                                                                                 `:=` (yearQuarter = zoo::as.yearqtr(format(paste0(enrollment_year, quarter)), "%Y%q"),
...
```{r, kyMedicaidAgg}
medicaidEnrollmentKyAgg <- medicaidEnrollmentKYQtr[ , .(enrolleesPerQtr = sum(totalEnrollees) / 3), by = .(state, yearQuarter)]

setorder(medicaidEnrollmentKyAgg, yearQuarter)

sdudTobaccoAggStateKy <- sdudTobaccoAggState[i = (state == 'KY')][, `:=` (stateFull = 'Kentucky')]

setorder(sdudTobaccoAggStateKy, yearQuarter)

sdudWMedicaid <- sdudTobaccoAggStateKy[medicaidEnrollmentKyAgg, on = .(yearQuarter)]

sdudMedicaidFinal <- sdudWMedicaid[, `:=` (ratePer1k = (totalRX / enrolleesPerQtr) * 1000)]
```
...
medicaidPopulationPlot <- sdudMedicaidFinal |> ggplot() +
  geom_col(aes(x = as.factor(yearQuarter), y = enrolleesPerQtr, fill = as.factor(yearQuarter)), color = 'black') +
  scale_fill_viridis_d() +
  theme_ipsum_rc(grid = 'Y') +
  theme(legend.position = 'none', axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14), axis.text.x = element_text(color = 'black'), axis.text.y = element_text(color = 'black')) +
...
jpeg(paste0('./plots/', format(Sys.Date(), '%Y%m%d'), "_ky-medicaid-pop.jpeg"), width = 18, height = 10, units = 'in', res = 1200)
medicaidPopulationPlot
dev.off()
```
...
chantixPer1k <- sdudMedicaidFinal |> ggplot() +
  geom_col(aes(x = as.factor(yearQuarter), y = ratePer1k, fill = as.factor(yearQuarter)), color = 'black') +
  scale_fill_viridis_d() +
  theme_ipsum_rc(grid = 'Y') +
  theme(legend.position = 'none', axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14), axis.text.x = element_text(color = 'black'), axis.text.y = element_text(color = 'black')) +
...
jpeg(paste0('./plots/', format(Sys.Date(), '%Y%m%d'), "_ky-varenicline-per-1k.jpeg"), width = 18, height = 10, units = 'in', res = 1200)
chantixPer1k
dev.off()
```